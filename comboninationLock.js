$(function(){
	...
	var combination = [4, 7, 1, 2];
	var lockid = "#lock1 ";

	var gridIncrement = $( lockid + ".lock-dial ul" ).css('line-height').replace('px', '')/2;
	var numNums = $( lockid + ".lock-dial:eq(0) ul li" ).length;
	var halfHeight = gridIncrement*numNums;
	var initTop = -(halfHeight-gridIncrement);

	$( lockid + ".lock-dial ul" ).css('top', initTop);

	$( lockid + ".lock-dial ul" ).draggable({
		...
		stop: function(){
			...
			for(var i=0; i<$( lockid + ".lock-dial ul" ).length; i++){
				comboArray[i] = $( lockid + ".lock-dial ul:eq("+i+")" ).attr("data-combo-num");
			}


			if(comboArray == ""+combination){
				$(lockid + '.lock-dial ul').draggable('disable');
				$(lockid + '#lock-wrapper').addClass("unlocked");
				$(lockid + '.lock-dial').each(function(){
					var $this = $(this);
